...
async function getRecentOutdoorData() {
  const seooul_collection = db.collection('SCollection');
  const seoul_data = await seooul_collection.findOne({}, { sort: { _id: -1 } });
  return seoul_data;
}
...
// 5분마다 외부 공기질 데이터 보내기
setInterval(async function () {
  const data = await getRecentOutdoorData();
  if (data) {
    sendData('/outdoor', data);
    console.log('outdoor to client:', data);
  } else {
    console.log('No new data');
  }
}, 5 * 1000);